  ...
  router.post('/login', async (req, res) => {
    try {
      ...
      const token = jwt.sign(
        { id: user.id, email: user.email, name: user.name, role: user.role || 'artist', tenantId: user.tenant_id },
        process.env.JWT_SECRET,
        { expiresIn: '24h' }
      );
      res.json({ token, user: { id: user.id, email: user.email, name: user.name, role: user.role || 'artist', tenantId: user.tenant_id } });
    } catch (err) {
  ...
  router.post('/register', async (req, res) => {
    try {
      ...
      const result = await pool.query(
        ...
        [email.toLowerCase().trim(), hashed, name.trim(), randomUUID()]
      );
      const user = result.rows[0];
      const token = jwt.sign(
      ...
      res.json({ token, user: { ...user, tenantId: user.tenant_id } });
    } catch (err) {
      if (err.code === '23505') {
