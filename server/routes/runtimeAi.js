...
module.exports = (pool) => {
  ...
  router.post('/scene-readiness', authenticate, async (req, res) => {
    try {
      ...
      const context = String(req.body?.context || 'Assess a lightweight interactive 3D scene for web and mobile spatial readiness.').slice(0, 4000);
      const providerResponse = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          model,
          messages: [
            { role: 'system', content: 'You are a spatial-computing architect. Give a concise readiness assessment covering geometry, performance, accessibility, and deployment risks.' },
      ...
      const stored = await pool.query(
        `INSERT INTO runtime_ai_results (user_id, feature, input, content, model, provider_receipt)
         VALUES ($1, 'scene-readiness', $2, $3, $4, $5) RETURNING id, created_at`,
      ...
      console.error('[runtime-ai] scene-readiness failed:', error.message);
      res.status(502).json({ error: 'OpenRouter scene readiness failed' });
    }
  });
  return router;
};
