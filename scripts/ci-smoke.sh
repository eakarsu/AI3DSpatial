...
(cd "$root" && node server/index.js) & pid=$!; trap 'kill "$pid" 2>/dev/null || true' EXIT
...
code="$(curl -sS -o /dev/null -w '%{http_code}' "http://127.0.0.1:$BACKEND_PORT/api/asset-conversion-workflows")"; test "$code" = 401
